...
// the grey placeholder eye sockets are exported separately so the page can fill
// them with the face tone and draw the white eyes on top.
...
const result = await page.evaluate(([edgeT, minSize]) => {
  ...
  return { W, H, regions: outR, silhouette: simplify(contour(sil), 2.5) };
}, [Number(edgeT), Number(minSize)]);
await browser.close();
const t = result;
// drop the grey placeholder eye sockets (light regions centred in the face band); kept aside for the page
const regions = t.regions.filter(r => !(r.grey > 90 && r.cy > 540 && r.cy < 760 && Math.abs(r.cx - AXIS) < 240 && r.poly.length > 6));
const socketRegions = t.regions.filter(r => !regions.includes(r));
// snap shared vertices: greedy clustering (no chaining)
const all = []; regions.forEach(r => r.poly.forEach(p => all.push(p))); socketRegions.forEach(r => r.poly.forEach(p => all.push(p))); t.silhouette.forEach(p => all.push(p));
...
const toPoly = r => dedupe(r.poly.map(p => snapped.get(p)));
const facets = regions.map(r => ({ grey: r.grey, poly: toPoly(r) })).filter(f => f.poly.length >= 3);
const sockets = socketRegions.map(r => ({ cx: r.cx, cy: r.cy, poly: toPoly(r) }));
const silhouette = dedupe(t.silhouette.map(p => snapped.get(p)));
